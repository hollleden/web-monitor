...
# ── Keyboards (updated style) ────────────────────────────────────────────────
def main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📋 My list"), KeyboardButton(text="💡 Help")],
...
def confirm_delete_keyboard(url_key: str):
    return InlineKeyboardMarkup(inline_keyboard=[[
        ...
        InlineKeyboardButton(text="❌ No, keep", callback_data="cancel_manage"),
    ]])
...
async def cmd_start(message: types.Message):
    ...
    # Format check frequency nicely
    if CHECK_EVERY >= 3600:
        hours = CHECK_EVERY // 3600
        freq = f"⏱ i check every {hours} hour" + ("s" if hours > 1 else "")
    else:
        minutes = CHECK_EVERY // 60
        freq = f"⏱ i check every {minutes} minute" + ("s" if minutes > 1 else "")
    await message.answer(
        f"🌙 <b>mzekali’s here.</b>\n\n"
...
# ── Check now with auto‑retry after cooldown ─────────────────────────────────
last_manual_check: float = 0
CHECK_NOW_COOLDOWN = 300  # 5 minutes
pending_auto_check: bool = False
auto_check_task: asyncio.Task | None = None

async def perform_check(message: types.Message | None = None):
    """Run the check and optionally send a result message."""
    rows = await get_urls()
    if not rows:
        if message:
    ...
    async with aiohttp.ClientSession() as session:
        for url_id, url, title, last_hash, last_text, is_up in rows:
            try:
                ...
                logging.error(f"check error for {url}: {e}")
    ...
    if parts:
        ...
        if status_msg:
            ...
            # auto‑check: send result with button for first change if only one
            if len(changes) == 1 and changes[0][1]:
                await bot.send_message(
                    ALLOWED_ID,
                    final_msg,
                    parse_mode="HTML",
                    reply_markup=open_site_keyboard(changes[0][1]),
                    disable_web_page_preview=True,
                )
            else:
                await bot.send_message(ALLOWED_ID, final_msg, parse_mode="HTML", disable_web_page_preview=True)
    else:
        if status_msg:
            await status_msg.edit_text(f"✅ done. {len(rows)} sites, nothing new.\n\n· {now_str()} ·", parse_mode="HTML")
        # else auto‑check with no changes → silent

async def delayed_check(delay_seconds: int):
    """Wait and then perform an automatic check."""
    global pending_auto_check, auto_check_task
    await asyncio.sleep(delay_seconds)
    pending_auto_check = False
    auto_check_task = None
    await perform_check(None)  # silent check with result message if changes

@dp.message(F.text == "🔄 Check now")
async def cmd_check_now(message: types.Message):
    global last_manual_check, pending_auto_check, auto_check_task
    if not only_me(message): return
    import time
    now_ts = time.time()
    if now_ts - last_manual_check < CHECK_NOW_COOLDOWN:
        remaining = CHECK_NOW_COOLDOWN - (now_ts - last_manual_check)
        wait_min = int(remaining / 60) + 1
        if not pending_auto_check:
            pending_auto_check = True
            auto_check_task = asyncio.create_task(delayed_check(remaining))
            await message.answer(f"⏳ i’ll check again automatically in ~{wait_min} min. sit tight 🌿")
        else:
            await message.answer(f"⏳ auto‑check already scheduled. just ~{wait_min} min left. 🌿")
        return
    last_manual_check = now_ts
    await perform_check(message)
...
async def delete_url(callback: types.CallbackQuery):
    ...
    success = await remove_url_by_key(url_key)

    if not success:
        await callback.answer("Site not found (maybe already removed).")
        # refresh the list view
        rows = await get_urls()
        if rows:
            await callback.message.edit_text(
                "❓ Site not found. Here’s the updated list:",
                parse_mode="HTML",
                reply_markup=list_view_keyboard(),
            )
            await cmd_list(callback.message)  # reuse the list display
        else:
            await callback.message.edit_text("📭 list is empty now.")
        return
